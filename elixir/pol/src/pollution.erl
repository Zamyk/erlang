-module(pollution).
-export([create_monitor/0, add_station/3, add_value/5, remove_value/4, get_one_value/4, get_station_mean/3, get_daily_mean/3, get_min_and_max/4, get_station_min/3]).
...
get_station_min(Name, Type, #monitor{records = Records}) ->
  maybe
    [_ | _] ?= Filtered = [ V || #value_key{type = T, station = S} := V <- Records, T == Type, S == Name],
    lists:min(Filtered)
  else
